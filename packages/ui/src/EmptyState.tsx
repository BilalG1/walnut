...
export function EmptyState({ title, hint, className }: EmptyStateProps) {
  return (
    <div className={cn('rounded-xl border border-dashed border-line px-6 py-12 text-center', className)}>
      <p className="text-sm font-medium text-fg-secondary">{title}</p>
      {hint !== undefined ? <p className="mx-auto mt-1 max-w-sm text-xs text-subtle">{hint}</p> : null}
    </div>
  )
}
